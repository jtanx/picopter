...
namespace waypoints_loop5_functions {
	void loadParameters(std::string fileName) {
		...
		parameters.insert("MIN_SPEED", &MIN_SPEED);
		parameters.insert("MAX_SPEED", &MAX_SPEED);
		...
		while(!exitProgram) {
			...
			switch(state) {
				...
				case 1:
					...
					flightVector = get_velocity(&controller, currentCoord, path, 10, MIN_SPEED, MAX_SPEED);
